...
//   - либо пипс переборный: from «ближе к выкиду» чем точный пункт пипса, и
//     выше from до конца дома нет занятых пунктов цвета c (from — самый дальний
//     от выкида занятый пункт; иначе переборным пипсом надо ходить дальней).
//
// TDD plan #21, #22, #23.
func IsLegalBearOff(b Board, c Color, from Point, pip uint8) bool {
	...
	// Переборный: from должен быть САМЫМ ДАЛЬНИМ от выкида занятым пунктом — выше
	// него (до конца дома) шашек этого цвета быть не должно. Проверять только до
	// exact мало: дальняя шашка может стоять ВЫШЕ точного пункта пипса (напр. на
	// 6 при пипсе 4) — тогда переборным пипсом ходить надо ей, а не выкидывать.
	switch c {
	case White:
		// from < exact (пипс перелетает from), дом 1..6, дальше = больший номер.
		if from >= exact {
			return false
		}
		for p := from + 1; p <= 6; p++ {
		...
		return true
	case Black:
		// Зеркально: дом 13..18, выкид в сторону меньших, дальше = больший номер.
		if from >= exact {
			return false
		}
		for p := from + 1; p <= 18; p++ {
